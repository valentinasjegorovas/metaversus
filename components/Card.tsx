...
const Card: React.FC<CardProps> = ({
  ...
  return (
    <div
      ...
      } flex cursor-pointer transition-[flex] duration-[0.7s] ease-out-flex min-w-[150px] bg-center bg-cover rounded-[24px] relative`}
      style={{ backgroundImage: `url(${imgUrl})` }}
      onClick={() => handleClick(id)}
    >
      {active !== id ? (
        <h3 className="text-white font-semibold text-[30px] rotate-[-90deg] absolute bottom-28 tablet:transform-none tablet:text-[18px] tablet:bottom-[10px] tablet:left-[15px]">
          {title}
        </h3>
      ) : (
        <div className="flex flex-col absolute bottom-0 pb-[48px] pl-[15px] tablet:pb-[10px]">
